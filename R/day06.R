...
#'
#' @param x some data
#' @param len length of sequence sought
#' @return For Part One and Two, `f06(x)` returns the last value of the first
#'     unique sequence of length `len`.
#' @export
#' @examples
#' f06(example_data_06(1), len = 4)
#' f06(example_data_06(1), len = 14)
f06 <- function(x, len = 4) {
  str_split(x, "") |>
    unlist() |>
    slider::slide(~ .x, .before = len - 1) |>
    lapply(\(x) length(unique(x)) == len) |>
    unlist() |>
    {\(x) which(x)}() |>
    min()
}
