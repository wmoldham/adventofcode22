...
#'
#' @param x some data
#' @return For Part One, `f02a(x)` returns the total score based on the first
#'   based on the first stratgy. For Part Two, `f02b(x)` returns the total score
...
f02a <- function(x) {
  df <-
    as_tibble(x) |>
    separate(value, c("opponent", "response"), sep = " ") |>
    mutate(
      opponent = factor(opponent, labels = c("rock", "paper", "scissors")),
      response = factor(response, labels = c("rock", "paper", "scissors")),
      selection = case_when(
        response == "rock" ~ 1,
        response == "paper" ~ 2,
        response == "scissors" ~ 3
      ...
      outcome = case_when(
        opponent == response ~ 3,
        opponent == "rock" & response == "paper" ~ 6,
        opponent == "rock" & response == "scissors" ~ 0,
        opponent == "paper" & response == "scissors" ~ 6,
        opponent == "paper" & response == "rock" ~ 0,
        opponent == "scissors" & response == "rock" ~ 6,
        opponent == "scissors" & response == "paper" ~ 0,
      )
    )
  sum(df$selection, df$outcome)
}
...
f02b <- function(x) {
  df <-
    as_tibble(x) |>
    separate(value, c("opponent", "outcome"), sep = " ") |>
    mutate(
      opponent = factor(opponent, labels = c("rock", "paper", "scissors")),
      outcome = factor(outcome, labels = c("lose", "draw", "win")),
      response = case_when(
        ...
        outcome == "draw" ~ as.character(opponent)
      ),
      selection = case_when(
        response == "rock" ~ 1,
        response == "paper" ~ 2,
