...
#'
#' @param x some data
#' @return For Part One, `f03a(x)` returns the total of priority scores for each
#'   rucksack. For Part Two, `f03b(x)` returns the total priority score for each
#'   grouping.
#' @export
#' @examples
...
f03a <- function(x) {
  ...
  first <-
    str_sub(x, 1, nchar(x) / 2) |>
    lapply(str_split_1, "")
  second <-
    str_sub(x, nchar(x) / 2 + 1, nchar(x)) |>
    lapply(str_split_1, "")
  shared <- map2_chr(first, second, intersect)
  map_dbl(shared, \(x) which(score %in% x)) |>
    sum()
}
...
f03b <- function(x) {
  score <- c(letters, LETTERS)
  groups <- split(x, ceiling(seq_along(x) / 3))
  map_chr(
    groups,
    ~ lapply(.x, str_split_1, "") |>
      reduce(intersect)
  ) |>
    map_dbl(\(x) which(score %in% x)) |>
    sum()
}
