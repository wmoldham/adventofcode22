...
#'
#' @param x some data
#' @return For Part One, `f09a(x)` returns the number of unique spaces that the
#'   tail visits in a two-knot chain. For Part Two, `f09b(x)` returns the number
#'   of unique spaces the tail visits in a ten-knot chain.
#' @export
#' @examples
#' f09a(example_data_09(1))
#' f09b(example_data_09(1))
f09a <- function(x) {
  f09_chase(x) |>
    hoist(tail1, tail_x = 1, tail_y = 2) |>
...
f09_follow <- function(tail, head) {
  if (max(abs(head - tail)) <= 1) {
    return(tail)
  }
  # move each tail coordinate one step toward head
  map2_dbl(tail, head, \(t, h) t + sign(h - t))
}

f09_chase <- function(x) {
  x |>
    tibble() |>
    extract(x, c("dir", "amount"), "(.*) (\\d+)", convert = TRUE) |>
    uncount(amount) |>
    inner_join(positions, by = "dir") |>
    mutate(
      head_x = cumsum(xd),
      head_y = cumsum(yd),
      head = map2(head_x, head_y, c),
      tail1 = accumulate(head, f09_follow, .init = c(0, 0))[-1]
    )
}
