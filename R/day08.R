...
#'
#' @param x some data
#' @return For Part One, `f08a(x)` returns the number of trees visible from outside
#'     the grid. For Part Two, `f08b(x)` returns the maximum visibility score of
...
f08a <- function(x) {
  ...
  queries <- rows * cols - edges
  out <- vector(length = queries)
  tree <- 1
  for (i in 2:(rows - 1)) {
    for (j in 2:(cols - 1)) {
      ht <- z[[i, j]]
      ...
      up <- z[1:(i-1), j]
      dn <- z[(i+1):nrow(z), j]
      is_visible <- all(ht > lt) || all(ht > rt) || all(ht > up) || all(ht > dn)
      out[[tree]] <- is_visible
      tree <- tree + 1
    }
  }
...
f08b <- function(x) {
  ...
  for (i in 2:(rows - 1)) {
    for (j in 2:(cols - 1)) {
      ...
      dn <- z[(i+1):nrow(z), j]
      area <-
        map_dbl(list(lt, rt, up, dn), f08_get_dist, ht) |>
        prod()
      out[[tree]] <- area
      tree <- tree + 1
    }
