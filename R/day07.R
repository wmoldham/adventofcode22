...
#'
#' @param x some data
#' @return For Part One, `f07a(x)` returns the total size of the directories smaller
#'     than 100000. For Part Two, `f07b(x)` returns the size of the smallest
#'     directory necessary to delete to make room for the new program.
#' @export
#' @examples
...
fmt_input <- function(x) {
  x |>
    as_tibble() |>
    extract(value, "cd_dir", "cd (.*)", remove = FALSE) |>
    mutate(path = c(accumulate(cd_dir, cd))) |>
    unnest(path) |>
    filter(str_detect(value, "\\d")) |>
    separate(value, c("size", "file"), sep = " ", convert = TRUE) |>
    group_by(path) |>
